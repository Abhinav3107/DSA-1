...
int poss_lines[15] = {12, 13, 14, 15, 16, 23, 24, 25, 26, 34, 35, 36, 45, 46, 56};
...
int max_ind(int array[], int size) {
    if (size <= 0) {
        return -1; // this is to Handle invalid size
    }
    ...
    for (int i = 1; i < size; i++) {
        if (array[i] > array[max_ind]) {
            max_ind = i; // Updatation of  maxIndex if a greater element is found
        }
    }
...
int find_ind(int array[], int size, int target_point) {
    for (int i = 0; i < size; i++) {
        if (array[i] == target_point) {
            return i;
        }
    }
    return -1;
...
move_t best_move(board_t board, player_t player) {
  ...
  for (int i = 0; i < 15; i++) {
    if (board[i] == NO) {
      ...
      player_t opposition = (player_t) player == RED ? BLUE : RED;

      // Compute the score for this move.
      move_t move_op = best_move(board, opposition);
      int score = move_op.score;


      if (has_won(board, opposition)){
  ...
  move_t best_move_val;
  best_move_val.line = best_mov_line;
  best_move_val.score = best_move_score;

  if (best_move_score == -1){
    best_move_val.line = max_ind(losing_moves_score, 15);
  }
...
int main() {
  ...
  while (1){
      ...
      else if (player_choise == 'b' || player_choise == 'B'){
          player = BLUE;
          break;
      }
      else {
  ...
  while (!is_full(board)){
    ...
    while (1){
        ...
        scanf("%d", &x);
        move = find_ind(poss_lines, 15, x);

        if (move == -1){
            printf("Invalid Input. Enter the smaller vertex number first. [Ex: 12, 13, 14]\n\n");
        }
        else if (board[move] != NO) {
            printf("cannot be reconnected. define two non-joined vertices.\n\n");
        }
        else {
            break;
        }
